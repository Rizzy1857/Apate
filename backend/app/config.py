...
class Config:
    ...
    def _load_from_file(self):
        ...
        try:
            ...
            if "database" in config_data:
                for k, v in config_data["database"].items():
                    if hasattr(self.database, k):
                        setattr(self.database, k, v)
            if "ai" in config_data:
                for k, v in config_data["ai"].items():
                    if hasattr(self.ai, k):
                        setattr(self.ai, k, v)
            if "honeypot" in config_data:
                for k, v in config_data["honeypot"].items():
                    if hasattr(self.honeypot, k):
                        setattr(self.honeypot, k, v)
            if "honeytokens" in config_data:
                for k, v in config_data["honeytokens"].items():
                    if hasattr(self.honeytokens, k):
                        setattr(self.honeytokens, k, v)
            if "logging" in config_data:
                for k, v in config_data["logging"].items():
                    if hasattr(self.logging, k):
                        setattr(self.logging, k, v)
            if "security" in config_data:
                for k, v in config_data["security"].items():
    ...
    def validate(self) -> List[str]:
        ...
        for p in [self.honeypot.ssh_port, self.honeypot.http_port, self.honeypot.tcp_port, self.honeypot.iot_port]:
            if not (1024 <= p <= 65535):
                issues.append("Ports must be between 1024 and 65535")
        ports = [self.honeypot.ssh_port, self.honeypot.http_port, self.honeypot.tcp_port, self.honeypot.iot_port]
